...
Cara paling umum untuk menambahkan CSS, adalah dengan menyimpan gaya di file CSS eksternal. Namun, dalam tutorial ini kita akan menggunakan gaya salto, karena ini lebih mudah untuk didemonstrasikan, dan lebih mudah bagi Anda untuk mencobanya sendiri.
...
Contoh berikut menyetel warna teks ```<h1>``` elemen menjadi biru, dan warna teks ```<p>``` elemen menjadi merah putih:
